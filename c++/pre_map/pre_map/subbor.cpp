...
void TestImAdjust(cv::Mat &mat_in, int low_in, int high_in, int low_out, int high_out){
    int rows = mat_in.rows;
    int cols = mat_in.cols;
    int type = mat_in.type();
    if(type == CV_8U){
        unsigned char *ptr = mat_in.data;
        for(int i = 0; i < rows * cols; ++i){
            unsigned char pixel = *ptr;
            double pixel_d = static_cast<double>(pixel);
            double pixel_out = (low_out + (high_out - low_out) * ((pixel_d - low_in) / (high_in - low_in)));
            if(pixel_out > 255){
                pixel_out = 255;
            }else if(pixel_out < 0){
                pixel_out = 0;
            }
            *ptr = static_cast<unsigned char>(pixel_out);
            ++ptr;
        }        
    }else if(type == CV_16U){
        unsigned short *ptr = reinterpret_cast<unsigned short *>(mat_in.data);
        for(int i = 0; i < rows * cols; ++i){
            ...
            *ptr =  static_cast<unsigned short>(pixel_out);
            ++ptr;
        }             
    }
}
