...
#            print local_real_node_list[i][2]
            local_real_node_list[i][2] = local_real_node_list[i][2] / self.resolution[0] - x_d
            local_real_node_list[i][3] = local_real_node_list[i][3] / self.resolution[1] - y_d
            local_real_node_list[i][4] = local_real_node_list[i][4] / self.resolution[2] - z_d
#            print local_real_node_list[i][2]
    ...
    def swc_list2line_list(self, swc_list):
        ...
        for i in xrange(len(swc_list)):
            if swc_list[i][6] == -1:
                ...
                line = Line(tmp_line, line_list, self.resolution)
...
class Line():
    def __init__(self, data, line_list, resolution):
        self.resolution = resolution
    ...
    def count_fiber_length(self):
        for i in range(self.length):
            if i == 0:
                ...
                l += math.sqrt(((this_node_x - pre_node_x) * self.resolution[0]) ** 2 + ((this_node_y - pre_node_y) * self.resolution[1]) ** 2 + ((this_node_z - pre_node_z) * self.resolution[2]) ** 2)
        return l
