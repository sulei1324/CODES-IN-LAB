...
class BouttonInteractorStyle(vtk.vtkInteractorStyleImage):
    ...
    def leftButtonPressEvent(self, obj, e):
        global main_win
        if main_win.toolBarAction2.isChecked():
    ...
    def sliceForward(self, obj, e):
        ...
        if cur_slice + 1 < cur_line_length:
            ...
            main_win.image_actor.SetInputData(main_win.imageLevelFilter.GetOutput())
...
#         self.loadFiles()
    ...
    def iniToorBar(self):
        ...
        self.toolBarAction1 = gui.QAction(self.toolBar)
        self.toolBarIcon1 = gui.QIcon(r'G:\sulei\workplace\bouttons\resource\f.png')
        self.toolBarAction1.setIcon(self.toolBarIcon1)
        self.toolBar.addAction(self.toolBarAction1)
        self.toolBarAction2 = gui.QAction(self.toolBar)
        self.toolBarIcon2 = gui.QIcon(r'G:\sulei\workplace\bouttons\resource\u.png')
        self.toolBarAction2.setIcon(self.toolBarIcon2)
        self.toolBar.addAction(self.toolBarAction2)        
        self.toolBarAction3 = gui.QAction(self.toolBar)
        self.toolBarIcon3 = gui.QIcon(r'G:\sulei\workplace\bouttons\resource\c.png')
        self.toolBarAction3.setIcon(self.toolBarIcon3)
        self.toolBar.addAction(self.toolBarAction3)
        self.toolBarAction4 = gui.QAction(self.toolBar)            
        self.toolBarIcon4 = gui.QIcon(r'G:\sulei\workplace\bouttons\resource\k.png')
        self.toolBarAction4.setIcon(self.toolBarIcon4)
        self.toolBar.addAction(self.toolBarAction4)
        self.toolBarAction1.triggered.connect(self.toolbarAction1Fun)
        self.toolBarAction1.setDisabled(True)
        self.toolBarAction2.setDisabled(True)
        self.toolBarAction3.setDisabled(True)
        self.toolBarAction4.triggered.connect(self.toolbarAction4Fun)
#         self.toolBarAction4.toggled.connect(self.toolbarAction4Fun)
        self.toolBarAction4.setDisabled(True)
        self.toolBarAction2.setCheckable(True)
        self.toolBarAction2.toggled.connect(self.toolbarAction2Fun2)
        self.toolBarAction3.triggered.connect(self.toolbarAction3Fun)
    ...
    def toolbarAction2Fun2(self):
        if not self.toolBarAction2.isChecked():
            if self.pickedActor:
            ...
            self.toolBarAction3.setEnabled(True)
 
    def toolbarAction1Fun (self):
    ...
    def iniVTK(self):
        ...
        self.image_actor.SetInputData(self.imageLevelFilter.GetOutput())
    ...
    def showImages(self):
        ...
        self.toolBarAction2.setEnabled(True)
        self.toolBarAction3.setEnabled(False)
    ...
    def loadFiles(self):
        ...
        if self.swc_src and self.mostd_src and not self.bouttons_src:
            self.toolBarAction1.setEnabled(True)
...
#         print real_load_num, len(self.lines[self.line_index])
        for i in xrange(real_load_num):
            ...
            z_s = int(z0)
            image_roi = (x_s, y_s, z_s, show_width, show_height, 1)            
            self.images[i] = self.loadImageFromMostd(image_roi)
        print time.clock() - t0
